...
def checkInner(data, sources, qX, qY, mean, stddev, pointer, thresh):
	count = 0
	...
	for i in range(len(sources)):
		...
		if sources['objc_type'][i] == 3 and sources['psfCounts'][i][1] < thresh[1] and sources['psfCounts'][i][2] < thresh[2] and sources['psfCounts'][i][3] < thresh[3]:
			#print(gthresh)
...
def begin(index):
	...
	try:
		...
		hdulist[0].header.append(('MEAN_BKG', obj_id, '3 sigma 5 iters mean background level'), end = True)
		hdulist[0].header.append(('MED_BKG', obj_id, '3 sigma 5 iters median background level'), end = True)
		hdulist[0].header.append(('STD_BKG', obj_id, '3 sigma 5 iters std of background level'), end = True)
		hdulist[0].header.append(('ZABS', obj_id, 'redshift of absorber'), end = True)
	except:
	...
	if inbounds(quasar['colc'][pointer] + chunk_size + 6, quasar['rowc'][pointer] + chunk_size + 6) and inbounds(quasar['colc'][pointer] - chunk_size - 5, quasar['rowc'][pointer] - chunk_size - 5):
		...
		print(thresh)
		image -= calc_background(scidata, xc, yc, int(400 / scale), int(500 / scale))
		...
		try:
			...
			#fits.writeto('/data/marvels/billzhu/2175 Quasar Cut/' + color + '/' + str(index) + '_DUST.fit', shifted, hdulist[0].header, overwrite = True)
			fits.writeto('/data/marvels/billzhu/MG II Quasar Cut/0.37 - 0.55/' + color + '/' + str(i) + '-' + color + '_MG.fit', shifted, hdulist[0].header, overwrite = True)
...
if __name__ == '__main__':
	...
	#gcheck_dirs = os.listdir('/data/marvels/billzhu/Reference Quasar Cut/0.37 - 0.55/g/')
	...
	#rangelist.append(gdirs[1].split('.')[0])
	...
	for d in gdirs:
		index = d.split('.')[0]
		#if d.split('.')[0] + '_REF.fit' not in gcheck_dirs:
