...
def absorber_frame(fwhm, flux, impact_parameter):
	FWHM = fwhm / 0.396
	plate_scale = 1
...
def psf_frame(fwhm, flux, comb_flux):
	FWHM = fwhm / 0.396
	plate_scale = 1
...
def begin(index):
	...
	color = index.split('-')[2]
	qso_flux = float(index.split('-')[3])
	index = str(i) + '-' + str(mgi) + '-' + color
	...
	for k in range(len(save_path)):
		...
		absorber_data = []
		for i in range(8):
			adi = absorber_frame(fwhm, qso_flux, i)
		...
		for offset in pixel:
			...
			comb_flux = photonCount(50, 50, fwhm / 0.396 * 1, adi)

			for i in range(10):
				psf = psf_frame(fwhm, qso_flux, comb_flux)
			...
			for j in range(len(largearr)):
				...
				new_comp.append(np.reshape(temp[44 : 57, 44 : 57], 169))
			...
			coeff = np.dot(np.reshape(adi[44 : 57, 44 : 57], 169), new_comp)
			...
			final_fit /= photonCount(50, 50, fwhm / 0.396, final_fit)
			final_fit *= photonCount(50, 50, fwhm / 0.396, adi)
			...
			try:
				...
				fits.writeto('/data/marvels/billzhu/Simulated PSF Subtract/' + save_path[k] + str(index) + '-' + str(offset) + '_SUB.fit', residue, clobber = True)
				fits.writeto('/data/marvels/billzhu/Simulated PSF Cut/' + save_path[k] + str(index) + '-' + str(offset) + '_PSF.fit', final_fit, clobber = True)
...
if __name__ == '__main__':
	...
	num_lines = sum(1 for line in open('2175 Reference QSO.txt'))
	check_dirs = os.listdir('/data/marvels/billzhu/Simulated PSF Subtract/r14/')
	print(check_dirs)
	print(len(check_dirs))
	counter = 0

	for i in range(num_lines):
		line = linecache.getline('2175 Reference QSO.txt', i + 1)
		line_data = line.split()
		#print(line_data)
		dust_index = int(line_data[0])
		catalog_index = int(line_data[2])
		qso_flux = TableDR12[catalog_index]['PSFFLUX'][2]

		if sum(str(dust_index) + '-' + str(catalog_index) + '-r' in s for s in check_dirs) < 8:
			rangelist.append(str(dust_index) + '-' + str(catalog_index) + '-r-' + str(qso_flux))
		else:
			counter += 1
			print(dust_index)
	

		#print(str(dust_index) + '-' + str(catalog_index) + '-r-' + str(qso_flux))
